...
\begin{itemize}\compresslist
	\vspace{-0.4em}
	\item Amplifying specific frequency bands according to a person's audiogram
	\item The ability of the user to select the direction in which they wish to listen and to hear sounds in that direction louder than those from other directions \cite{Evaluation_of_a_hearing_compensation_algorithm}
\end{itemize}
...
\headerbox{System Design}{name=method,column=0,below=objectives, above=bottom}{ % This block's bottom aligns with the bottom of the conclusion block
	...
	\begin{center}
		...
		\captionof{figure}{Hearing aid system overview \cite{Evaluation_of_digital_hearing_aid_algorithms}}
	\end{center}
	...
	\begin{center}
		...
		\resizebox{\columnwidth}{!}{%
			\begin{tabular}{ccc}
				...
				Number of steerable angles & 19 (10\textsuperscript{o} increments) & 5 (0\textsuperscript{o}, 60\textsuperscript{o}, 90\textsuperscript{o}, 120\textsuperscript{o}, 180\textsuperscript{o}) \\ 
				\bottomrule
			\end{tabular} 
		}
	\end{center}
...
\headerbox{Results: Hardware}{name=results3,column=2,row=0}{
	Compensatory gain error was calculated using the difference between the desired and measured gain at the centre of each frequency band
	\begin{center}
		\captionof{table}{Error in compensatory gain}
			\begin{tabular}{cc}
				\toprule
				\textbf{Frequency band (kHz)} &\textbf{Error (\%)} \\ 
				\midrule
				2.8 - 3.5 & 0.81 \\ 
				5.6 - 7 & 19.56 \\ 
				\bottomrule
...
The error in the beam steering ability of the directionality feature was calculated by comparing the measured and the simulated polar plots for each steerable angle
\begin{center}
	\captionof{table}{Average error in directionality}
\begin{tabular}{cc}
	\toprule
	\textbf{Angle (\textsuperscript{o})} & \textbf{Average Error (\%)} \\
	\midrule 
	0 & 46.6 \\ 
	60 & 30.7 \\ 
	90 & 12.7 \\ 
...
\begin{itemize}\compresslist
	...
	\item Making use of more microphones to improve the precision of the beam steering in the directionality feature
...
This project is a proof of concept that an inexpensive, fully functional, adaptive hearing aid can be produced.
This was proven by:

\begin{itemize}\compresslist
	\item Amplifying specific frequency bands in accordance with individual audiograms
	\item Steering the acoustic beam to attenuate signals in directions other than the selected one
	\item Creating a hearing aid that costs R1500
\end{itemize}

Overall device accuracy:
\begin{itemize}\compresslist
		\item Average error in the measured gain when compared to the desired gain is 10.2\%
	\item Average error between simulated and measured steered beam is 32.9\% 
\end{itemize}
}
...
\begin{center}
	\includegraphics[width=0.5\linewidth]{fftSoftware.png}
...
The polar plot shows the normalised magnitudes of the signal at various angles, with respect to the user, when the dial is facing $90^{o}$
	...
	\captionof{figure}{Gain applied to the output signal when the beam is steered to 90\textsuperscript{o}}
\end{center}
...
The effects of compensatory gain are illustrated below through the application of different amplifications to the two frequency bands
	\begin{center}
		\includegraphics[width=0.5\linewidth]{hardwareGain.png}
